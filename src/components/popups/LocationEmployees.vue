<template>
    <DefaultPopup :is-shown="isShown" confirmLabel="Видалити" title="Видалення користувача" @close="close"
        :show-confirm-button="false">
        <template v-slot:body>
            <div class="location-employees" v-if="employees.length">
                <SmallUserPreviewCard v-for="employee in employees" :key="employee.name"
                    :name="`${employee.name} ${employee.surname}`" :id="employee._id" :unique-id="employee.uniqueId" />
            </div>
...
export default {
    ...
    mounted() {
        this.$store.dispatch('getLocationEmployeesAction', { id: this.id }).then(res => {
            if (res.success) {
